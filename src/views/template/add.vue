  ...
  <div class="is-footer">
    ...
    <div class="footer-btn">
      ...
      <el-button
        ...
        >提交</el-button
      >
      <!-- <el-button v-if="id" type="primary" size="lg" :disabled="formData.powerList.length === 0" :loading="saveLoading" @click="save(true)">另存为模板</el-button> -->
      <el-button
        v-if="id"
        type="primary"
        size="lg"
    ...
    <el-dialog title="创建模板" :visible.sync="dialogFormVisible" width="500px">
      <el-form ref="subForm" :model="subForm" :rules="formRules" label-width="100px">
        <el-form-item label="模板名称：" prop="templateName">
          <el-input
            v-model.trim="subForm.templateName"
            :maxlength="50"
            autocomplete="off"
            placeholder="请输入模板名称"
          />
        ...
        <el-form-item label="状态：" prop="status">
          <el-select v-model="subForm.status" placeholder="请选择模板状态" style="width: 100%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :loading="saveAsLoading" type="primary" @click="handleSetTemplateBaseInfo">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    // 获取模板详情
    async getDetailsServiceTemplate() {
      if (!this.id) {
        return
      }
      const result = await templateApi.getDetailsServiceTemplate(this.id)
    ...
    async getServiceList() {
      ...
      this.getDetailsServiceTemplate()
    },
    // 选择服务
    changeService(e) {
      this.formData.serviceId = e
      this.formData.serviceName = _.find(this.serviceList, { value: e }).label
    ...
    changeChecked(evenv, item, type, parentItem) {
      let powerListIds = _.map(this.formData.powerList, 'powerId')
      let newItem = {}
      if (type === 'all') {
        // 全选
        ...
        if (evenv) {
          item.children.map(e => {
            ...
            if (index < 0) {
              this.formData.powerList.push(newItem)
              powerListIds.push(e.id)
            } else {
              this.formData.powerList[index] = newItem
            }
          })
        } else {
          const powerIds = _.map(item.children, 'id')
          powerListIds = _.difference(powerListIds, powerIds)
          this.formData.powerList = _.filter(this.formData.powerList, e => !powerIds.includes(e.powerId))
        }
      } else {
        // 单选
        if (evenv) {
    ...
    async saveSuccess(type) {
      try {
        ...
        if (type || !this.id) {
          await templateApi.addServiceTemplate(this.formData)
        } else {
          await templateApi.updateServiceTemplate(this.formData)
        }
        if (type) {
